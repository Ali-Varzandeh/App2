  ...
  app.get("/basket", userIsAuthorized, (req, res) => {
    ...
    connection.query(query, function (error, results, fields) {
      ...
      res.render("basket", {
        ...
        user: req.user,
        basket: results,
      }); // it take the result of the query and insert to the basket template and generate HTML and send it to the browser
    });
  });
};
...
// displays all user's orders
const ordersRoute = (app, connection) => {
...
const deleteFromBasketRoute = (app, connection) => {
  app.post(
    "/product/delete/:products_baskets_id",
    userIsAuthorized,
    (req, res) => {
      const basket_id = req.user.ID;
